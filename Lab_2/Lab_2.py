...
def true_solution(x):
   ...
   def u2(x):
      return C3*np.exp(-x) + C4*np.exp(x) + np.sin(np.pi*x)/((ksi**2)*(1 + np.pi**2)) 
   x = np.linspace(0, 0.3, 11) 
   plt.plot(x, u1,'o-', linewidth = 2.0, label='траектория до точки разрыва')
   x = np.linspace(0.3, 1, 11) 
   plt.plot(x, u2,'o-', linewidth = 2.0, label='траетория после точки разрыва')
   plt.legend()
   plt.xlabel('x')
   plt.ylabel('u(x)')
   plt.grid()
...
def test_matrix():
   ...
   for i in range(n - 1):
      B[i] = a[i + 1]*((n + 1)**2)
      C[i] = (a[i] + a[i + 1])*((n + 1)**2) + d[i]

def test_progon(A, B, C):
   #! Прямая прогонка
   alfa = np.empty(n)
   beta = np.empty(n)
   alfa[0] = kappa[0]
   ...
   for i in range(n - 1):
      ...
      beta[i + 1] = (fi[i] + A[i]*B[i])/(C[i] - A[i]*alfa[i])
   #! Обратная прогонка
   y = np.empty(n)
