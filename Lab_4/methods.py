...
def Chebyshev(n, m, eps, Nmax, cheb, V, R, task):
    ...
    --------------------------------------
    Принимает размер сетки, критерий точности, число итераций, параметр k, массив невязок и численного решения
    
    Возвращает численное решение и невязку на S, так же невязку на нулевом шаге и выход по числу шагов или точности
    
    Реализовывается сам численный метод
    """
    h = (b - a)/n
    k = (d - c)/m
...
def MVR(n, m, eps, Nmax, omega, V, R, task):
    ...
    ------------------------
    Принимает размер сетки, критерий точности, число итераций, параметр omega, массив невязок и численного решения
...
def MPI_standart(n, m, eps, Nmax, V, R, task):
    """
    Метод Простой Итерации для стандартной области
    ----------------------------------------------
    Принимает размер сетки, критерий точности, число итераций, массив невязок и численного решения
    
    Возвращает численное решение и невязку на S, так же невязку на нулевом шаге и выход по числу шагов или точности, и параметр tau
...
def MPI_non_standart(n, m, eps, Nmax):
    """
    Метод Простой Итерации для нестандартной области
    ------------------------------------------------
    Принимает размер сетки, критерий точности, число итераций
    
    Возвращает полностью решенную задачу, то есть создает все необходимые данные и запускает численный метод, 
    после чего рисует графики, создает таблицы и выводит справку
    
    Реализовывается сама задача
    """
    U, V, R, Z = [], [], [], []
    a, c = -1, -1
    b, d = 1, 1
